...
class ProbSpace:
    ...
    def filterDat(self, filtSpec, minPoints=None, maxPoints=None, adat=None):
        ...
        if DEBUG and progressive:
            print('attempt = ', attempt, ', delta = ', delta, ', remaining = ', remaining, ', minVal, maxVal = ',
                  minPoints, maxPoints)
    ...
    def prob(self, targetSpec, givenSpec=None):
        ...
        if type(targetSpec) == type([]):
            ...
            assert len(
                targetSpecU) == 0, 'prob.P: All target specifications must be bound (i.e. specified as tuples).  For unbound returns, use distr.)'
    ...
    def distr_experimental(self, rvName, givenSpecs=None, power=None):
        ...
        if not givenSpecs:
            ...
            filtSpace = self.SubSpace(filtSpecs, density=self.density, power=self.power, minPoints=100,
                                      maxPoints=sqrt(self.N))
            if not condSpecs:
    ...
    def dependence(self, rv1, rv2, givensSpec=[], power=None, raw=False):
        ...
        for spec in condFiltSpecs:
            ...
            for testSpec in testSpecs:
                ...
                numTests += 1
                if DEBUG:
                    print('spec = ', spec, testSpec, ', givensB = ', givensB, ', dep = ', dep, ', prob1.N, prob2.N = ',
                          prob1.N, prob2.N)
