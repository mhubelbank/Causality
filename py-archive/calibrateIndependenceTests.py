...
FILES = ['..\Tests\synthTest1', '..\Tests\synthTest2', '..\Tests\synthTest2a', '..\Tests\synthTest4', '..\Tests\synthTest8']
#FILES = ['..\Tests\synthTest2']
TEST_TYPES = ['IndHSIC', 'IndLn', 'IndMI', 'IndGauss']
#TEST_TYPES = ['IndGauss']
...
class Calibrator:
		...
		for testType in TEST_TYPES:
			...
			for i in range(runs):
				...
				for filePath in FILES:
					self.calibrateOneIndependenceTest(testType, filePath)
	...
	def calibrateOneIndependenceTest(self, testType, filePath):
		...
		dependencies, independencies = SA.getDependencies()
		...
		for item in dependencies:
			...
			ind = module.isIndependent(X,Y)
			if ind:
				#print('Error -- ', x, 'and', y, 'Should be dependent')
				self.err1Count += 1
				errors += 1
				errorTerms[item]=1
			self.testCount += 1
		for item in independencies:
			...
			ind = module.isIndependent(X,Y)
			if not ind:
				#print('Error -- ', x, 'and', y, 'Should be independent')
				self.err2Count += 1
				errors += 1
				errorTerms[item] = 1
			self.testCount += 1
		#print('Rating = ', (1 - (errors / items))*100, '%')
		#CIs = SA.getCondIndependencies()
		#print('CIs = ', CIs)
